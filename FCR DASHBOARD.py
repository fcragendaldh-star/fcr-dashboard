# app.py
"""
Streamlit FCR Dashboard
Reads cleaned daily Excel files from a folder and shows:
...
# ---------- CONFIG ----------
# Support environment variable for data folder (useful for deployment)
DATA_FOLDER_PATH = os.getenv("DATA_FOLDER", "data")
DATA_FOLDER = Path(DATA_FOLDER_PATH)

# Create data folder if it doesn't exist (important for deployment)
DATA_FOLDER.mkdir(parents=True, exist_ok=True)
logger.info(f"Data folder set to: {DATA_FOLDER.absolute()}")
...
def _load_all_files_uncached(folder_path: str) -> pd.DataFrame:
    ...
    # Create folder if it doesn't exist (important for deployment)
    ...
    for f in files:
        ...
        try:
            ...
            expected_cols = ["Sr No", "Sub Division", "Tehsil/Sub Tehsil", "Officer",
...
def load_all_files(folder_path: str, cache_version: str = "v1") -> pd.DataFrame:
    ...
    cache_key is used to force cache refresh when files change.
    """
    folder = Path(folder_path)
    # Create folder if it doesn't exist (important for deployment)
    folder.mkdir(parents=True, exist_ok=True)
    ...
    for f in files:
        ...
        try:
            ...
            # Normalize header if file contains extra rows; assume headers present
            # Ensure expected columns exist. If not, try simple heuristics.
            expected_cols = ["Sr No", "Sub Division", "Tehsil/Sub Tehsil", "Officer",
                             "Uncontested Pendency", "Income Certificate", "Copying Service",
                             "Inspection Records", "Overdue Mortgage", "Overdue Court Orders",
                             "Overdue Fardbadars"]
            
            # If column names differ, try to map by fuzzy match: lower stripped
            df_cols = {c.lower().strip(): c for c in df.columns}
    ...
    # Standardize typical columns
    if "Total" not in combined.columns and "TOTAL" in combined.columns:
        combined.rename(columns={"TOTAL": "Total"}, inplace=True)
    
    # Define actual pendency columns (based on updated Excel structure)
    pendency_columns = [
        "Uncontested Pendency", "Income Certificate", "Copying Service",
        "Inspection Records", "Overdue Mortgage", "Overdue Court Orders",
        "Overdue Fardbadars"
...
# Official Title - Subtle
st.markdown("""
<div style="background-color: #f8f9fa; padding: 1rem 1.5rem; margin-bottom: 1.5rem; border-bottom: 2px solid #003366;">
...
# Generate cache key based on files in folder to detect new files
...
if refresh or st.session_state.get("force_uncached_load", False):
    ...
    with st.spinner("Loading data (fresh reload)..."):
        df_all = _load_all_files_uncached(str(DATA_FOLDER))
    ...
    with st.spinner("Loading data..."):
        df_all = load_all_files(str(DATA_FOLDER), cache_version=st.session_state.cache_version)


# Update file hash after successful load
if not df_all.empty and DATA_FOLDER.exists() and current_file_hash:
    ...
    if latest_snapshot.empty:
        ...
        with col_left:
            ...
            if not top3_subdivisions.empty:
                for idx, row in top3_subdivisions.iterrows():
